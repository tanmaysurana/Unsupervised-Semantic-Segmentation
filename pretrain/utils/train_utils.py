...
def train(p, train_loader, model, optimizer, epoch, amp):
    ...
    for i, batch in enumerate(train_loader):
        ...
        im_q_labels = torch.Tensor(batch['query']['label']).cuda(p['gpu'], non_blocking=True)
        print(im_q_labels)
        ...
        logits, labels, saliency_loss = model(im_q=im_q, im_k=im_k, sal_q=sal_q, sal_k=sal_k, im_q_labels=im_q_labels)
